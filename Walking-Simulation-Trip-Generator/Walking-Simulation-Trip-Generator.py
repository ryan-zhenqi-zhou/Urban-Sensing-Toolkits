...
class CrawlWindow(QDialog):
    def __init__(self):
        ...
        self.setWindowTitle('步行模拟出行生成器')

        self.fangfa_label = QLabel('使用方法：在C盘根目录下放入walk.csv文件，格式为“起点百度经度,起点百度纬度,终点百度经度,终点百度纬度”四列，输入AK，点击软件界面的开始爬取后将会将结果输出在与软件相同目录下的shp文件夹中。', self)
    ...
    def table_init(self):
        ...
        self.table.setHorizontalHeaderLabels(['ori_lng_bd', 'ori_lat_bd', 'des_lng_bd', 'des_lat_bd','ori_lng_wgs84', 'ori_lat_wgs84', 'des_lng_wgs84', 'des_lat_wgs84', 'distance','duration'])
    ...
    def set_table_slot(self,ori_lng_bd,ori_lat_bd,des_lng_bd,des_lat_bd,ori_lng_wgs84,ori_lat_wgs84,des_lng_wgs84,des_lat_wgs84,distance,duration):
        ...
        self.table.setItem(row, 4, QTableWidgetItem(ori_lng_wgs84))
        self.table.setItem(row, 5, QTableWidgetItem(ori_lat_wgs84))
        self.table.setItem(row, 6, QTableWidgetItem(des_lng_wgs84))
        self.table.setItem(row, 7, QTableWidgetItem(des_lat_wgs84))
        self.table.setItem(row, 8, QTableWidgetItem(distance))
        self.table.setItem(row, 9, QTableWidgetItem(duration))
...
class CrawlThread(QThread):
        ...
        try:
            ...
            if dt['message'] == 'ok':
                ...
                jss2 = [float(Ori.split(',')[0]), float(Ori.split(',')[1]),
                        ...
                        float(distance), float(duration), path_wgs84]
                return jss2
            else:
                pass
        except:
    ...
    def run1(self,ak):
        ...
        for x in range(len(a_1)):
            ...
            self.get_data2(str(Ori), str(Des), str(ak))
            if jss2 != []:
                jss3.append(jss2)
                y = y + 1
        ...
        try:
            ...
            df1.rename(
                columns={0: 'ori_lng_bd', 1: 'ori_lat_bd', 2: 'des_lng_bd', 3: 'des_lat_bd', 4: 'ori_lng_wgs84',
                         5: 'ori_lat_wgs84',6: 'des_lng_wgs84',7: 'des_lat_wgs84',8: 'distance',9: 'duration',
                         10: 'path_wgs84'}, inplace=True)

            if len(df1) == 0:
                self.log_signal.emit('<font color="red">出现错误！</font>')
                self.log_signal.emit('<font color="red">请输入正确的AK！</font>')
            else:
                self.log_signal.emit('总共生成{:.0f}条步行路径'.format(len(df1)))
            dv = df1.reset_index()
            for b in range(len(dv)):
                self.result_signal.emit(str(dv['ori_lng_bd'][b]),str(dv['ori_lat_bd'][b]),str(dv['des_lng_bd'][b]),str(dv['des_lat_bd'][b]),
                                        str(dv['ori_lng_wgs84'][b]),str(dv['ori_lat_wgs84'][b]),str(dv['des_lng_wgs84'][b]),str(dv['des_lat_wgs84'][b]),
                                        str(dv['distance'][b]),str(dv['duration'][b]))
            # 生成shp点o文件
            wo = shapefile.Writer(r'.\shp\point_o')
            wo.field('ori_lng_wgs84', 'F', decimal=10)
            wo.field('ori_lat_wgs84', 'F', decimal=10)
            for num in range(len(dv)):
                wo.point(dv['ori_lng_wgs84'][num],dv['ori_lat_wgs84'][num])
                wo.record(
                    dv['des_lng_wgs84'][num],
                    dv['des_lat_wgs84'][num])
            wo.close()
            # 生成shp点d文件
            wd = shapefile.Writer(r'.\shp\point_d')
            wd.field('des_lng_wgs84', 'F', decimal=10)
            wd.field('des_lat_wgs84', 'F', decimal=10)

            for num in range(len(dv)):
                wd.point(dv['des_lng_wgs84'][num],dv['des_lat_wgs84'][num])
                wd.record(
                    dv['des_lng_wgs84'][num],
                    dv['des_lat_wgs84'][num])
            wd.close()
            # 生成shp线文件
            w = shapefile.Writer(r'.\shp\walk lines')
            ...
            w.field('ori_lng_wgs84', 'F', decimal=10)
            w.field('ori_lat_wgs84', 'F', decimal=10)
            w.field('des_lng_wgs84', 'F', decimal=10)
            w.field('des_lat_wgs84', 'F', decimal=10)
            w.field('distance', 'F')
            w.field('duration', 'F')
            ...
            for num in range(len(dv)):
                ...
                w.record(
                    ...
                    dv['des_lng_bd'][num],
                    dv['des_lat_bd'][num],
                    dv['ori_lng_wgs84'][num],
                    dv['ori_lat_wgs84'][num],
                    dv['des_lng_wgs84'][num],
                    dv['des_lat_wgs84'][num],
                    dv['distance'][num],
                    dv['duration'][num])
