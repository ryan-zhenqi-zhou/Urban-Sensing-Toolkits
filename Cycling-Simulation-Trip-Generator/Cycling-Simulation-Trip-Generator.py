...
class CrawlWindow(QDialog):
    def __init__(self):
        ...
        self.setWindowTitle('骑行模拟出行生成器')

        self.fangfa_label = QLabel('使用方法：在C盘根目录下放入ride.csv文件，格式为“起点百度经度,起点百度纬度,终点百度经度,终点百度纬度”四列，输入AK，点击软件界面的开始爬取后将会将结果输出在与软件相同目录下的shp文件夹中。', self)
        ...
        self.xuanze_label = QLabel('0-自行车 1-电瓶车', self)
        self.xuanze_line = QLineEdit(self)
    ...
    def table_init(self):
        ...
        self.table.setHorizontalHeaderLabels(['ori_lng_bd', 'ori_lat_bd', 'des_lng_bd', 'des_lat_bd', 'distance','duration','turn'])
        self.table.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)

    def layout_init(self):
        self.koujian_h_layout.addWidget(self.xuanze_label)
        self.koujian_h_layout.addWidget(self.xuanze_line)
    ...
    def btn_slot(self, btn):
        if btn == self.start_btn:
            ...
            self.xuanze_line.setEnabled(False)
            self.start_btn.setEnabled(False)
            ...
            self.xuanze_line.setEnabled(True)
            self.stop_btn.setEnabled(False)
    ...
    def set_table_slot(self,ori_lng_bd,ori_lat_bd,des_lng_bd,des_lat_bd,distance,duration,turn):
        ...
        self.table.setItem(row, 6, QTableWidgetItem(turn))

    def lineedit1_init(self):
        self.ak_line.setPlaceholderText('请输入AK')
        self.xuanze_line.setPlaceholderText('请输入0或1，0代表自行车，1代表电瓶车')

        self.ak_line.textChanged.connect(self.check_input1_func)
        self.xuanze_line.textChanged.connect(self.check_input1_func)

    def check_input1_func(self):
        if self.ak_line.text() and self.xuanze_line.text():
            self.start_btn.setEnabled(True)
        else:
            self.start_btn.setEnabled(False)
...
class CrawlThread(QThread):
    ...
    # 骑行路径规划
    def get_data2(self,xuanze,Ori,Des,ak):
        global jss2
        jss2 = []
        try:
            ...
            url = 'http://api.map.baidu.com/direction/v2/riding?origin={}&destination={}&ak={}&riding_type={}'. \
                format(Ori1, Des1, ak, xuanze)
            ...
            if dt['message'] == 'ok':
                ...
                jss2 = [float(Ori.split(',')[0]), float(Ori.split(',')[1]),
                        ...
                        float(distance), float(duration), turn, path_wgs84]
                return jss2
            else:
                pass
        except:
    ...
    def run1(self,ak,xuanze):
        ...
        for x in range(len(a_1)):
            ...
            self.get_data2(str(xuanze),str(Ori), str(Des), str(ak))
            if jss2 != []:
                jss3.append(jss2)
                y = y + 1
                self.log_signal.emit('正在爬取第{:.0f}条骑行路径'.format(y))
        return jss3

    # 运行函数1
    def run(self):
        ak_o = self.Crawl_Window.ak_line.text()
        xuanze_o = self.Crawl_Window.xuanze_line.text()
        try:
            self.run1(str(ak_o),str(xuanze_o))
            df1 = pd.DataFrame(jss3)
            df1.rename(
                columns={0: 'ori_lng_bd', 1: 'ori_lat_bd', 2: 'des_lng_bd', 3: 'des_lat_bd', 4: 'ori_lng_wgs84',
                         5: 'ori_lat_wgs84',6: 'des_lng_wgs84',7: 'des_lat_wgs84',8: 'distance',9: 'duration',10: 'turn',
                         11: 'path_wgs84'}, inplace=True)

            if len(df1) == 0:
                self.log_signal.emit('<font color="red">出现错误！</font>')
                self.log_signal.emit('<font color="red">请输入正确的AK和骑行方式！</font>')
            else:
                self.log_signal.emit('总共生成{:.0f}条骑行路径'.format(len(df1)))
            dv = df1.reset_index()
            for b in range(len(dv)):
                self.result_signal.emit(str(dv['ori_lng_bd'][b]),str(dv['ori_lat_bd'][b]),str(dv['des_lng_bd'][b]),str(dv['des_lat_bd'][b]),
                                        str(dv['distance'][b]),str(dv['duration'][b]),str(dv['turn'][b]))
            # 生成shp线文件
            w = shapefile.Writer(r'.\shp\ride lines')
