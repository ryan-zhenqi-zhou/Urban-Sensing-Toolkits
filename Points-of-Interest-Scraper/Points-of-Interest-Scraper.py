...
class CrawlWindow(QDialog):
    ...
    def lineedit1_init(self):
        ...
        self.leibie_line.setPlaceholderText('请输入POI分类')
...
class CrawlThread(QThread):
                ...
                if 'results' in data:
                    if data['total'] != 0:
                        for item in data['results']:
                            ...
                            jss1.append(jss)
                            y = y+1
                            self.log_signal.emit('正在爬取第{:.0f}个POI'.format(y))
                    else:
                        break
                else:
                    pass
    ...
    def run(self):
        ...
        try:
            self.get_data(zuoxia_o, youshang_o, leibie_o, ak_o)
            df1 = pd.DataFrame(jss1)
            '''
            'name', 'lng_bd', 'lat_bd', 'lng_wgs84', 'lat_wgs84','address','uid','province','city','area'
            '''
            df1.rename(
            ...
            if len(df1) == 0:
                self.log_signal.emit('<font color="red">总共生成0个POI,请重新输入正确的左下点坐标、右上点坐标、分类、AK！若还是如此，此区域无此类POI</font>')
            else:
                self.log_signal.emit('去掉重复POI,总共生成{:.0f}个有效POI'.format(len(df1)))
            dv = df1.reset_index()
            ...
            for num in range(len(dv)):
                wd.point(dv['lng_wgs84'][num],dv['lat_wgs84'][num])
                wd.record(
                    dv['name'][num],
                    dv['lng_bd'][num],
                    dv['lat_bd'][num],
