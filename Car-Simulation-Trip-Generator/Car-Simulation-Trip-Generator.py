...
class CrawlWindow(QDialog):
    def __init__(self):
        ...
        self.setWindowTitle('车行模拟出行生成器')

        self.fangfa_label = QLabel('使用方法：在C盘根目录下放入car.csv文件，格式为“起点百度经度,起点百度纬度,终点百度经度,终点百度纬度”四列，输入策略（0：默认，3：不走高速，4：高速优先，5：躲避拥堵，6：少收费，7：躲避拥堵&高速优先，8：躲避拥堵&', self)
        ...
        self.fangfa1_label = QLabel('不走高速，9：躲避拥堵&少收费，10：躲避拥堵&不走高速&少收费，11：不走高速&少收费），输入汽车类型（0：普通汽车，1：纯电动汽车），输入AK，点击软件界面的开始爬取后将会将结果输出在与软件相同目录下的shp文件夹中。', self)
        ...
        self.tactics_label = QLabel('策略:', self)
        self.tactics_line = QLineEdit(self)
        self.cartype_label = QLabel('汽车类型:', self)
        self.cartype_line = QLineEdit(self)
    ...
    def table_init(self):
        ...
        self.table.setHorizontalHeaderLabels(['ori_lng_bd', 'ori_lat_bd', 'des_lng_bd', 'des_lat_bd', 'distance','duration','taxi_fee','toll'])
        self.table.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)

    def layout_init(self):
        self.v_layout.addWidget(self.fangfa_label)
        self.v_layout.addWidget(self.fangfa1_label)
        self.koujian_h_layout.addWidget(self.tactics_label)
        self.koujian_h_layout.addWidget(self.tactics_line)
        self.koujian_h_layout.addWidget(self.cartype_label)
        self.koujian_h_layout.addWidget(self.cartype_line)
    ...
    def btn_slot(self, btn):
        if btn == self.start_btn:
            ...
            self.tactics_line.setEnabled(False)
            self.cartype_line.setEnabled(False)
            self.start_btn.setEnabled(False)
            ...
            self.tactics_line.setEnabled(True)
            self.cartype_line.setEnabled(True)
            self.stop_btn.setEnabled(False)
    ...
    def set_table_slot(self,ori_lng_bd,ori_lat_bd,des_lng_bd,des_lat_bd,distance,duration,taxi_fee,toll):
        ...
        self.table.setItem(row, 6, QTableWidgetItem(taxi_fee))
        self.table.setItem(row, 7, QTableWidgetItem(toll))

    def lineedit1_init(self):
        self.ak_line.setPlaceholderText('请输入AK')
        self.tactics_line.setPlaceholderText('请输入策略')
        self.cartype_line.setPlaceholderText('请输入汽车类型')

        self.ak_line.textChanged.connect(self.check_input1_func)
        self.tactics_line.textChanged.connect(self.check_input1_func)
        self.cartype_line.textChanged.connect(self.check_input1_func)

    def check_input1_func(self):
        if self.ak_line.text() and self.tactics_line.text() and self.cartype_line.text():
            self.start_btn.setEnabled(True)
        else:
            self.start_btn.setEnabled(False)
...
class CrawlThread(QThread):
    ...
    # 车行路径规划
    def get_data2(self, Ori, Des, ak, tactics, cartype):
        global jss2
        jss2 = []
        try:
            ...
            url = 'http://api.map.baidu.com/direction/v2/driving?origin={}&destination={}&ak={}&tactics={}&cartype={}'. \
                format(Ori1, Des1, ak, tactics, cartype)
            ...
            if dt['message'] == '成功':
                ...
                taxi_fee = dt['result']['routes'][0]['taxi_fee']
                toll = dt['result']['routes'][0]['toll']
                ...
                jss2 = [float(Ori.split(',')[0]), float(Ori.split(',')[1]),
                        ...
                        float(distance), float(duration), float(taxi_fee), float(toll), path_wgs84]
                return jss2
            else:
                pass
        except:
    ...
    # 运行函数2
    def run1(self,ak, tactics, cartype):
        p = open("C:\\car.csv")
        ...
        for x in range(len(a_1)):
            ...
            Des_2 = a_1[x].split(',')[3]
            Des = Des_1 + ',' + Des_2
            self.get_data2(str(Ori), str(Des), str(ak),str(tactics), str(cartype))
            if jss2 != []:
                jss3.append(jss2)
                y = y + 1
                self.log_signal.emit('正在爬取第{:.0f}条车行路径'.format(y))
        return jss3

    # 运行函数1
    def run(self):
        ak_o = self.Crawl_Window.ak_line.text()
        tactics_o = self.Crawl_Window.tactics_line.text()
        cartype_o = self.Crawl_Window.cartype_line.text()
        try:
            self.run1(str(ak_o),str(tactics_o),str(cartype_o))
            df1 = pd.DataFrame(jss3)
            df1.rename(
                columns={0: 'ori_lng_bd', 1: 'ori_lat_bd', 2: 'des_lng_bd', 3: 'des_lat_bd', 4: 'ori_lng_wgs84',
                         5: 'ori_lat_wgs84',6: 'des_lng_wgs84',7: 'des_lat_wgs84',8: 'distance',9: 'duration',10:'taxi_fee',11:'toll',
                         12: 'path_wgs84'}, inplace=True)

            if len(df1) == 0:
                self.log_signal.emit('<font color="red">出现错误！</font>')
                self.log_signal.emit('<font color="red">请输入正确的策略编码，车辆类型编码和AK！</font>')
            else:
                self.log_signal.emit('总共生成{:.0f}条车行路径'.format(len(df1)))
            dv = df1.reset_index()
            for b in range(len(dv)):
                self.result_signal.emit(str(dv['ori_lng_bd'][b]),str(dv['ori_lat_bd'][b]),str(dv['des_lng_bd'][b]),str(dv['des_lat_bd'][b]),
                                        str(dv['distance'][b]),str(dv['duration'][b]),str(dv['taxi_fee'][b]),str(dv['toll'][b]))

            # 生成shp线文件
            w = shapefile.Writer(r'.\shp\car lines')
